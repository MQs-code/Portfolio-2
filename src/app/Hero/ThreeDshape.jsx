...
// 1. ADDED: A clean, instant black fallback to prevent the "white flash"
const SceneFallback = () => (
  ...
  return (
    <Float speed={2} rotationIntensity={1.5} floatIntensity={2} position={responsivePosition}>
      <mesh ref={meshRef} scale={responsiveScale}>
        ...
        {type === "torus" && <torusGeometry args={[0.8, 0.25, 8, 32]} />} {/* FURTHER OPTIMIZED segments */}
        ...
        <meshStandardMaterial
          ...
          flatShading={isMobile} // SPEED BOOST: Uses simpler shading on mobile
        />
      </mesh>
    </Float>
  );
    ...
    <div className="relative w-full h-screen bg-[#050505] overflow-hidden">
      <div className="absolute inset-0 z-0">
        <Canvas 
          ...
          // OPTIMIZATION: Fixed DPR for mobile, high performance power preference
          dpr={typeof window !== 'undefined' && window.innerWidth < 768 ? 1 : [1, 1.5]}
          gl={{ 
            antialias: false, // Turn off for max speed
            powerPreference: "high-performance",
            precision: "lowp" // Drastically faster math for mobile GPUs
          }}
          style={{ background: '#050505' }}
        >
          {/* Use the new fallback here */}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-6 pointer-events-none">
        <motion.div
          // REMOVED viewport logic for instant mount
        ...
        >
          ...
          <div className="pt-6 md:pt-10 pointer-events-auto">
            <Link
              href="/contact"
              className="group relative inline-flex items-center justify-center px-10 md:px-16 py-3.5 md:py-5 overflow-hidden rounded-xl bg-emerald-500 transition-all duration-500 active:scale-95 touch-manipulation"
            >
              <span className="relative z-10 text-black font-black text-[11px] md:text-[13px] uppercase tracking-[0.3em]">
                Get In Touch
              </span>
              <div className="absolute inset-0 translate-y-full bg-white transition-transform duration-500 lg:group-hover:translate-y-0 hidden lg:block" />
            </Link>
          </div>
        </motion.div>
      </div>
